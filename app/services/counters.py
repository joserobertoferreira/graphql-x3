...
from app.core import local_menus
from app.core.settings import DB_SCHEMA, DEFAULT_LEGACY_DATE


def get_next_counter(
    db: Session, counter_code: str, site: str = '', date: date = DEFAULT_LEGACY_DATE, comp: str = ''
) -> str:
    """
    ...
    Args:
        ...
        date (date, optional): The date for the counter. Defaults to DEFAULT_LEGACY_DATE.
    ...
    try:
        ...
        stmt_select = f'SELECT * FROM {DB_SCHEMA}.ACODNUM WHERE CODNUM_0 = :code'

        result = db.execute(stmt_select, {'code': counter_code})
        counter_data = result.one_or_none()

        if counter_data is None:
            raise ValueError(f'Contador {counter_code} não encontrado.')

    except Exception as e:
        print(f'Erro ao obter código do contador (sync): {e}')
        return ''
    ...
    try:
        ...
        counter = _create_next_counter(db, counter_code, site_or_society, period, comp, counter_data['LNG_0'])

        # Generate the final counter string
        if counter:
            final_counter = _attrib_number(counter, counter_data)
        ...
        print(f'Erro ao obter próximo editor_id (sync): {e}')
...
def _attrib_number(counter: str, counter_data: Row, date: date) -> str:
    """
    ...
    for i in range(0, counter_data['NBPOS_0']):
    ...
    if counter_data['TYP_0'] == local_menus.Chapter46.NUMERIC:
...
def _for_month(counter_data: Row, index: int, date: date) -> str:
    ...
    """
    if counter_data[f'POSLNG_{index}'] == 2:
        return f'{date.month:02d}'
    elif counter_data[f'POSLNG_{index}'] == 3:
...
def _create_next_counter(db: Session, counter_code: str, **kwargs) -> str:
    ...
    stmt_select = text(
        f'SELECT VALEUR_0 FROM {DB_SCHEMA}.AVALNUM '
        f'WHERE CODNUM_0 = :code AND SITE_0 = :site '
        f'AND PERIOD_0 = :period AND COMP_0 = :comp'
    )
    ...
    # Atualiza o contador na tabela
    if current_value > 1:
        stmt_update = text(
            f'UPDATE {DB_SCHEMA}.AVALNUM SET VALEUR_0 = :next_val '
            f'WHERE CODNUM_0 = :code AND SITE_0 = :site '
            f'AND PERIOD_0 = :period AND COMP_0 = :comp'
        )

        try:
            db.execute(stmt_update, {'next_val': next_value, 'code': counter_code})
        except Exception as e:
            raise ValueError(f'Erro ao atualizar o contador {counter_code} (sync): {e}')
    else:
        stmt_insert = text(
            f'INSERT INTO {DB_SCHEMA}.AVALNUM ('
            f'CODNUM_0, SITE_0, PERIOD_0, COMP_0, VALEUR_0, '
        ...
        try:
            ...
            raise ValueError(f'Erro ao inserir o contador {counter_code} (sync): {e}')
...
def _check_value_in_columns(
    row: Row, value_to_find: Any, search_column: str = '', limit_column: str = ''
    ...
    try:
        limits_to_check = int(limit)
    except (ValueError, TypeError):
        print(f"Erro: Não foi possível converter o valor da coluna limite '{limit_column}' ('{limit}') para inteiro.")
        return False, 0

    if limits_to_check <= 0:
    ...
    for i in range(limits_to_check):  # Itera de 0 até limits_to_check - 1
        ...
        if column_to_check not in row_keys:
            print(
                (
                    ...
                    f'(baseado em {limit_column}={limits_to_check}) não encontrada na linha.'
                )
            )
            continue
    ...
    print(
        f'Valor {value_to_find} não encontrado nas colunas {search_column}_0 a {search_column}_{limits_to_check - 1}.'
    )
